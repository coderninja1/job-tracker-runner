 ...
 * @author Andrew Popp
 */
@Table(uniqueConstraints = {@UniqueConstraint(columnNames = {"name", "job_id", "purchaseOrder_id"})})
public class Task extends NamedEntity<Integer> {
    ...
    public static Task newInstance(String name, String description, String taskNumber, Job job, PurchaseOrder purchaseOrder, TaskStatus taskStatus) {
        Task task = new Task();
        task.setName(name);
        task.setDescription(description);
        task.setTaskNumber(taskNumber);
        task.setPurchaseOrder(purchaseOrder);
        task.setJob(job);
    ...
    public void validate() {
        ...
        if (getPurchaseOrder() == null) {
            throwSystemException("Related Task [" + getName() + "], Task Number [" + getTaskNumber() + "] is missing a PurchaseOrder. Please ensure the Task has a PurchaseOrder selected.");
        }
        getPurchaseOrder().validate();
    }
